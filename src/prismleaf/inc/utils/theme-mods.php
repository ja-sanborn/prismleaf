...
if ( ! function_exists( 'prismleaf_get_header_icon_pixel_size' ) ) {
	/**
	 * Get the pixel size of the header icon based on the selected size.
	 *
	 * @since 1.0.0
	 *
	 * @param string|null $size Optional size slug. Defaults to the theme mod value.
	 * @return int
	 */
	function prismleaf_get_header_icon_pixel_size( $size = null ) {
		$slug = $size;
		if ( null === $slug ) {
			$slug = prismleaf_get_theme_mod_header_icon_size();
		}
...
if ( ! function_exists( 'prismleaf_get_theme_mod_pagination_shape' ) ) {
	...
	function prismleaf_get_theme_mod_pagination_shape( $setting_id, $default_key = '', $default_value = 'Round' ) {
		...
		if ( '' === $setting_id ) {
			return prismleaf_sanitize_pagination_shape( $default_value );
		}

		$raw = prismleaf_get_theme_mod( $setting_id, $default_value );
		return prismleaf_sanitize_pagination_shape( $raw );
	}
}

if ( ! function_exists( 'prismleaf_get_theme_mod_author_avatar_shape' ) ) {
	/**
	 * Retrieve a sanitized author avatar shape selection.
	 *
	 * @since 1.0.0
	 *
	 * @param string $setting_id Theme mod ID.
	...
	function prismleaf_get_theme_mod_author_avatar_shape( $setting_id, $default_key, $default_value = 'Circle' ) {
		...
		if ( '' === $setting_id ) {
			return prismleaf_sanitize_author_avatar_shape( $default_value );
		}
